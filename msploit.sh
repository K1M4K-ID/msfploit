...
if [ -f /usr/bin/apktool ];
then
	sleep 0.1
else
	rm /root/.local/share/apktool/framework/1.apk &> /dev/null;sleep 0.1
	tar -xvf apktool.tar.gz &> /dev/null;chmod 777 apktool*;mv -f apktool* /usr/bin
	sleep 0.1
	rm -r apktool.tar.gz &> /dev/null
fi
# check zipalign is installed
if [ -f /usr/bin/zipalign ];
then
	sleep 0.1
else
	...
	rm -r zipalign_8.1.0+r23-2_arm64.deb &> /dev/null
	sleep 0.1
fi
...
function sets_apsinal(){
        ...
        printf "\033[37;1m[\033[32;1m*\033[37;1m] local ip address anda, wlan0 (\033[31;1m$wlan0_ip\033[37;1m)\033[31;1m\n\n"
